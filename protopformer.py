...
class PPNet(nn.Module):
    ...
    def conv_features(self, x, reserve_layer_nums=[]):
        ...
        if "Vision" in feature_module_name or "MyCait" in feature_module_name:
            ...
            token_attn = None
            x, (cls_token_attn, _) = self.features.forward_feature_mask_train_direct(
                cls_embed, x_embed, token_attn, reserve_layer_nums
            )
            final_reserve_num = reserve_layer_nums[-1][1]
            final_reserve_indices = torch.topk(
                cls_token_attn, k=final_reserve_num, dim=-1
            )[
                1
            ]  # (B, final_reserve_num)
    ...
    def get_PPC_loss(self, total_proto_act, cls_attn_rollout, original_fea_len, label):
        ...
        discrete_values = discrete_values[None, :, :].repeat(
            batch_size * proto_per_class, 1, 1
        )  # (B * 10, 196, 2)
        discrete_weights = torch.zeros(
            batch_size, proto_per_class, original_fea_len
        ).cuda()  # (B, 10, 196)
        ...
        proto_indices = (
            (label * proto_per_class).unsqueeze(dim=-1).repeat(1, proto_per_class)
        )
        ...
        discrete_weights.scatter_(
            2, reserve_token_indices, total_proto_act
        )  #   (B, 10, 196)
        discrete_weights = discrete_weights.reshape(
            batch_size * proto_per_class, -1
        )  # (B * 10, 196)
    ...
    def forward(self, x):
        ...
        if not self.training:
            if self.use_global:
                (cls_tokens, img_tokens), (token_attn, cls_token_attn, _) = (
        ...
        if self.use_global:
            (cls_tokens, img_tokens), (student_token_attn, cls_attn_rollout, _) = (
                self.prototype_distances(x, reserve_layer_nums)
            )
...
class BaseNet(nn.Module):
    ...
    def __init__(
        ...
        self.features = base_architecture_to_features[base_architecture](
        ...
        if "deit" in base_architecture:
            first_add_on_layer_in_channels = [
                i for i in self.features.modules() if isinstance(i, nn.Linear)
            ][-1].out_features
        ...
        self.last_layer = nn.Linear(
            first_add_on_layer_in_channels, self.num_classes, bias=True
        )  # do not use bias
        if init_weights:
            self._initialize_weights()
    ...
    def push_forward(self, x):
        ...
        fea_width, fea_height = int(fea_len ** (1 / 2)), int(fea_len ** (1 / 2))
        conv_features = conv_features.permute(0, 2, 1).reshape(
            (B, dim, fea_height, fea_width)
        )
...
def construct_PPNet(
    ...
    if (
        "deit" in base_architecture
        or "pit" in base_architecture
        or "tnt" in base_architecture
        or "cait" in base_architecture
    ):
