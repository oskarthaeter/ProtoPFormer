...
def get_discard_img(view_img, discard_indices, fea_size, patch_size, replace_color):
    ...
    for discard_indice in discard_indices:
        ...
        res_img[
            indice_h * patch_size : (indice_h + 1) * patch_size,
            indice_w * patch_size : (indice_w + 1) * patch_size,
        ] = replace_color
    return res_img


def find_high_activation_crop(activation_map, percentile=95):
    # print("activation_map", activation_map, activation_map.shape)
...
def get_gaussian_params(proto_act, scale_coe=0.9):
    ...
    discrete_values = np.array(
        [[x, y] for x in range(fea_size) for y in range(fea_size)]
    )
...
def get_closest_patches(ppnet, prototype_vectors, data_loader, patch_size=16):
    closest_patches_info = [(-1, -1, -1, float("inf"))] * prototype_vectors.shape[0]

    for img_batch, _, indices in tqdm(data_loader):
        img_batch = img_batch.cuda()
        with torch.no_grad():
            (cls_tokens, img_tokens), _ = ppnet.conv_features(
            ...
            distances = distances.view(
                img_tokens.shape[0], img_tokens.shape[1], img_tokens.shape[2], -1
            )

            for i in range(prototype_vectors.shape[0]):
                min_dist = torch.min(distances[..., i])
                min_dist_indices = torch.nonzero(
                    distances[..., i] == min_dist, as_tuple=False
                )
                batch_idx, h_idx, w_idx = min_dist_indices[0]
                # are the h_idx and w_idx in the correct range?
                if (h_idx + 1) * patch_size >= 224 or (w_idx + 1) * patch_size >= 224:
                    raise ValueError(
                        f"Patch indices out of range for batch {batch_idx}: h_idx={h_idx}, w_idx={w_idx}"
                    )
                img_idx = indices[batch_idx].item()

                if min_dist.item() < closest_patches_info[i][-1]:
                    closest_patches_info[i] = (
                        int(img_idx),
                        int(h_idx),
                        int(w_idx),
                        min_dist.item(),
    ...
    return closest_patches_info


def extract_patch(image, h_idx, w_idx, patch_size):
    # print(image.shape)
    # print(h_idx * patch_size, (h_idx + 1) * patch_size)
    return image[
        :,
...
def visualize_prototypes(closest_patches_info, dataset, save_path, patch_size=16):
    os.makedirs(save_path, exist_ok=True)
    transform = [
        torchvision.transforms.v2.PILToTensor(),
        torchvision.transforms.v2.Resize((256, 256), interpolation=3),
        torchvision.transforms.v2.CenterCrop((224, 224)),
    ]
    transform = torchvision.transforms.v2.Compose(transform)
    for i, (img_idx, h_idx, w_idx, dist) in enumerate(closest_patches_info):
        path = os.path.join(
            dataset.root, dataset.base_folder, dataset.data.iloc[img_idx].filepath
        )
        image_pil = PIL.Image.open(path)
        # Resize to 224x224
        image = transform(image_pil)

        patch = extract_patch(image, h_idx, w_idx, patch_size)
        patch = patch / 255.0
        patch = torchvision.transforms.v2.functional.to_pil_image(patch)

        # Convert the original image for plotting
        image_pil = torchvision.transforms.v2.functional.to_pil_image(image)

        # Draw bounding box on the original image
        draw = PIL.ImageDraw.Draw(image_pil)
        draw.rectangle(
            [
                (w_idx * patch_size, h_idx * patch_size),
                ((w_idx + 1) * patch_size, (h_idx + 1) * patch_size),
            ],
            outline="red",
            width=3,
        )

        # Plot the original image and the patch side by side
        ...
        plt.tight_layout()
        plt.savefig(f"{save_path}/prototype_{i}.png")
        plt.close()
...
def visualize_prototype(
    ...
    # Get the target prototype vector
    target_prototype = ppnet.prototype_vectors[prototype_idx].detach().cuda()

    with tqdm(range(num_steps)) as pbar:
        for _ in pbar:
            optimizer.zero_grad()

            # Pass the input image through the model to get its feature representation
            (cls_tokens, img_tokens), _ = ppnet.conv_features(
                input_img, ppnet.reserve_layer_nums
            )

            # Calculate the distance to the target prototype
            img_tokens = img_tokens.permute(0, 2, 3, 1)  # [B, H, W, C]
            distances = torch.cdist(
                img_tokens.view(-1, img_tokens.shape[-1]), target_prototype.view(1, -1)
            )
...
def save_fig(X, Y, Z, save_path):
    ...
    surf = ax1.plot_surface(
        X, Y, Z, rstride=3, cstride=3, linewidth=2, antialiased=True, cmap=cm.viridis
    )
...
def get_args():
    ...
    parser.add_argument(
        "--prototype_shape", nargs="+", type=int, default=[2000, 192, 1, 1]
    )
