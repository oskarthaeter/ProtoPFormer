...
def train_one_epoch(
    ...
    for samples, targets, indices in metric_logger.log_every(
            ...
            if args.use_ppc_loss:
                ...
                if hasattr(model, "module"):
                    ppc_cov_loss, ppc_mean_loss = model.module.get_PPC_loss(
                        total_proto_act, cls_attn_rollout, original_fea_len, targets
                    )
                else:
                    ppc_cov_loss, ppc_mean_loss = model.get_PPC_loss(
                        total_proto_act, cls_attn_rollout, original_fea_len, targets
                    )
