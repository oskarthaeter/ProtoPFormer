...
class MyVisionTransformer(VisionTransformer):
    ...
    def attn_rollout(self, all_attn, discard_ratio=0.9, head_fusion="mean"):
        result = (
            torch.eye(all_attn[0].shape[-1])
            .unsqueeze(dim=0)
            .repeat(all_attn[0].shape[0], 1, 1)
            .cuda()
        )
        for attn in all_attn:
            ...
            identity_w = 0.2
            a = (attn_fused + identity_w * I) / (1.0 + identity_w)
    ...
    def forward_feature_mask_train_thresh(
        self, cls_embed, x_embed, token_attn=None, reserve_layer_nums=[]
    ):
        """
        cls_embed : (B, 1, dim)
        x_embed : (B, 196, dim)
        ...
        policy = torch.ones(
            B, 1 + patch_num, 1, device=x_embed.device
        )  # (B, 1 + 196, 1)
        ...
        all_attn = all_attn[
            : layer_ids[0]
        ]  # select the first several layers (196 * 196)
        attn_rollout = self.attn_rollout(all_attn)
    ...
    def forward_feature_mask_train_direct(
        self, cls_embed, x_embed, token_attn=None, reserve_layer_nums=[]
    ):
        """
        ...
        for i, blk in enumerate(self.blocks):
            if i in layer_ids:
                ...
                reserve_token_indice = torch.topk(
                    cls_token_attn, k=reserve_token_num, dim=-1
                )[
                    1
                ]  # (B, reserve_token_num)
                ...
                policy = torch.cat(
                    [
                        ...
                        torch.zeros(B, patch_num, device=x.device),
                    ],
                    dim=1,
                )
