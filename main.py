...
def get_args_parser():
    parser = argparse.ArgumentParser(
        "Vision Transformer KD training and evaluation script", add_help=False
    )
    ...
    # ProtoPFormer
    parser.add_argument(
        "--base_architecture", type=str, default="deit_tiny_patch16_224"
    )
    ...
    parser.add_argument("--ppc_cov_thresh", type=float, default=1.0)
    parser.add_argument("--ppc_mean_thresh", type=float, default=2.0)
    ...
    parser.add_argument(
        "--save_ep_freq", default=400, type=int, help="save epoch frequency"
    )
...
def get_outlog(args):
    ...
    tb_writer = SummaryWriter(
        log_dir=os.path.join(tb_dir, args.model + "_" + args.data_set), flush_secs=1
    )
    logger = utils.get_logger(
        level=logging.INFO,
        mode="w",
        name=None,
        logger_fp=os.path.join(logfile_dir, args.model + "_" + args.data_set + ".log"),
    )
...
def main(args):
    ...
    if args.enable_smoothing:
        ...
        logger.info(
            "Label smoothing is enabled, smoothing rate: {}".format(args.smoothing)
        )
    elif not args.enable_smoothing:
        assert args.smoothing == 0
        logger.info(
            "Label smoothing is not enabled, smoothing rate: {}".format(args.smoothing)
        )
    ...
    optimizer = create_optimizer(
        args, model_without_ddp, joint_optimizer_lrs=joint_optimizer_lrs
    )
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_stats = train_one_epoch(
            ...
            args=args,
            tb_writer=tb_writer,
            iteration=__global_values__["it"],
        ...
        if args.output_dir:
            if (epoch + 1) % args.save_ep_freq == 0:
                checkpoint_paths = [
                    output_dir / "checkpoints/checkpoint-{}.pth".format(epoch)
                ]
        ...
        test_stats = evaluate(
            data_loader=data_loader_val, model=model, device=device, args=args
        )
